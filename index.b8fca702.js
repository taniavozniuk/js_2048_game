...
startButton.addEventListener("click", ()=>{
    if (game.getStatus() === "idle") game.start();
        ...
        if (messageLose2) messageLose2.style.display = "none";
    ...
    if (messageStart) messageStart.style.display = "none";
...
function updateCell(cell, num) {
    ...
    if (num > 0) {
        ...
        if (num <= 1024) cell.classList.add(`field-cell--${num}`);
...
function showWinMessage() {
    ...
    if (messangeWim) messangeWim.classList.remove("hidden");
}
...
function checkGameOver(board) {
    ...
    for(let r = 0; r < rows; r++)for(let c = 0; c < columns; c++){
        if (board[r][c] === 0) return false; // Є порожня клітинка, гра продовжується
...
class Game {
   ...
   * Starts the game.
   */ start() {
        if (this.status !== "idle" && this.status !== "lose") return; // Забороняємо перезапуск, якщо гра ще триває або вже виграна
        this.status = "playing";
   ...
   */ getState() {
    ...
    calculateBoardSum() {
        return this.board.flat().filter(Boolean).reduce((acc, val)=>acc + val, 0);
    }
    /**
   ...
   */ addNewNumber() {
        ...
        for(let rowIdx = 0; rowIdx < this.rows; rowIdx++){
            for(let colIdx = 0; colIdx < this.columns; colIdx++)if (this.board[rowIdx][colIdx] === 0) emptyCells.push({
                row: rowIdx,
                col: colIdx
            });
        }
   ...
   */ slideRowLeft(row) {
        ...
        while(merged.length < this.columns)merged.push(0);
        return {
            newRow: merged,
            scoreGained
        };
   ...
   */ moveRight() {
        ...
        this.board = this.board.map((row)=>{
            ...
            if (reversedRow.toString() !== originalRow.toString()) moved = true;
            totalScoreGained += scoreGained;
            return reversedRow;
        });
        if (moved) {
   ...
   */ transposeBoard() {
        ...
        for(let r = 0; r < this.rows; r++)for(let c = 0; c < this.columns; c++)transposed[c][r] = this.board[r][c];
   ...
   */ updateStatus() {
        let hasEmpty = false;
        let canMerge = false;
        for(let rowIdx = 0; rowIdx < this.rows; rowIdx++)for(let colIdx = 0; colIdx < this.columns; colIdx++){
            if (this.board[rowIdx][colIdx] === 0) hasEmpty = true;
            if (colIdx < this.columns - 1 && this.board[rowIdx][colIdx] === this.board[rowIdx][colIdx + 1] || rowIdx < this.rows - 1 && this.board[rowIdx][colIdx] === this.board[rowIdx + 1][colIdx]) canMerge = true;
   ...
   */ hasMoves() {
        for(let rowIdx = 0; rowIdx < this.rows; rowIdx++)for(let colIdx = 0; colIdx < this.columns; colIdx++){
            if (this.board[rowIdx][colIdx] === 0) return true; // Є порожня клітинка
            if (colIdx < this.columns - 1 && this.board[rowIdx][colIdx] === this.board[rowIdx][colIdx + 1] || rowIdx < this.rows - 1 && this.board[rowIdx][colIdx] === this.board[rowIdx + 1][colIdx]) return true; // Можливе злиття
        }
        return false;
    }
}
